...
void voiceWordsCallback(const std_msgs::String::ConstPtr &msg)
{
	...
	if (dataString.find("你是谁") != std::string::npos || dataString.find("名字") != std::string::npos)
	{
		char nameString[100] = "我是你的语音小助手，你可以叫我小R";
		text = nameString;
		std::cout << text << std::endl;
	}
	else if (dataString.find("你几岁了") != std::string::npos || dataString.find("年龄") != std::string::npos)
	{
		char eageString[100] = "我已经四岁了，不再是两三岁的小孩子了";
		text = eageString;
		std::cout << text << std::endl;
	}
	else if (dataString.find("你可以做什么") != std::string::npos || dataString.find("干什么") != std::string::npos)
	{
	...
	FILE *pipe = popen("play tts_sample.wav", "r");
	if (pipe == nullptr)
	{
		// 处理错误
		perror("popen");
...
{
	int ret = MSP_SUCCESS;
	const char *login_params = "appid = 3ab73ec8, work_dir = ."; // 登录参数,appid与msc库绑定,请勿随意改动
	/*
	 * rdn:           合成音频数字发音方式
	 * volume:        合成音频的音量
	 * pitch:         合成音频的音调
